...
// same solution as 123.best-time-to-buy-and-sell-stock-iii. Make the initial remainTransaction == k instead of 2. Modify the size of DP table.

// subproblems: maxProfitAtDayI(isHolding, day, remainTransaction): starting from `day`, with/without stock, with a remainTransaction times, find the maximum profit I can achieve. Goal: maxProfitAtDayI(isHolding=false, day=0, remainTransaction=k).
...
// 	return maxProfitAtDayI(prices, k-1, 0, 0, table)
// }
...
func maxProfit(k int, prices []int) int {
	// DP table [remainTransaction=k + 1 padding][isHolding=2][day]
	table := make([][][]int, k+1)
	for i, _ := range table {
		table[i] = make([][]int, 2)
		for j, _ := range table[i] {
	...
	for remainTransaction := 1; remainTransaction <= k; remainTransaction++ {
